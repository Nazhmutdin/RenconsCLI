...
class WelderDataExtractionService:
    def __init__(self) -> None:
        self.methods = {
            "Толщина деталей, мм": self.__extract_details_thikness,
            "Наружный диаметр, мм": self.__extract_outer_diameter,
    ...
    def extract_from_main_page(self, main_page: str) -> list[dict[str, str | date | None]]:
        result = []
        ...
        for tr in trs:
            ...
            expiration_date = str_to_date(row_tree.xpath("//tr[@bgcolor]/td[10]/text()")[0].strip())
            expiration_date_fact = str_to_date(row_tree.xpath("//tr[@bgcolor]/td[11]/text()")[0].strip())
            additional_id = row_tree.xpath("//tr[@bgcolor]//a/@onclick")[0].strip().split("detail.php?ID=")[1].replace('"', '').split(",")[0]
    ...
    def extract_from_additional_page(self, additional_page: str) -> dict[str, str]:
        result = {}
        tree: html.HtmlElement = html.fromstring(additional_page)
